...
def render_card(stats: AgentStats, theme: str = "dark") -> str:
    ...
    for i, (label, value) in enumerate(metrics):
        ...
        panels.append(
            f'<rect x="{x}" y="{y}" width="219" height="66" rx="9" fill="{c["panel"]}" {motion("reveal", i)}/>'
        )
        body.append(
    ...
    return (
        frame(495, 236, "Vibe Clock Stats", subtitle, theme)
        + "\n".join(panels) + sheen("reveal", range(4), 495, 236, opacity=0.2)
        + "\n".join(body) + "</svg>"
    )
